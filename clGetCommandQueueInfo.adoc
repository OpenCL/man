...
|Return Type and Information returned in param_value
...
The reference count returned with `CL_QUEUE_REFERENCE_COUNT` should be considered immediately stale.
...
These properties are specified by the value associated with the `CL_COMMAND_QUEUE_PROPERTIES` passed in `properties` argument in <<clCreateCommandQueueWithProperties.adoc#, `clCreateCommandQueueWithProperties`>>.
...
<<clCreateCommandQueueWithProperties.adoc#, `clCreateCommandQueueWithProperties`>>, <<clReleaseCommandQueue.adoc#, `clReleaseCommandQueue`>>, <<clRetainCommandQueue.adoc#, `clRetainCommandQueue`>>
