...
The OpenCL context associated with `command_queue`, `src_buffer`, and `dst_image` must be the same.
...
If the mipmap extensions are enabled with <<cl_khr_mipmap_image.adoc#, `cl_khr_mipmap_image`>>, calls to <<clEnqueueCopyImage.adoc#, `clEnqueueCopyImage`>>, <<clEnqueueCopyImageToBuffer.adoc#, `clEnqueueCopyImageToBuffer`>>, and `clEnqueueCopyBufferToImage` can also be used to copy from and to a specific mip-level of a mip-mapped image.
...
* `CL_INVALID_MEM_OBJECT` if `src_buffer` is not a valid buffer object and `dst_image` is not a valid image object or if `dst_image` is a 1D image buffer object created from `src_buffer`.

* `CL_INVALID_VALUE` if the 1D, 2D, or 3D rectangular region specified by `dst_origin` and `dst_origin` + `region` refers to a region outside `dst_origin`, or if the region specified by `src_offset` and `src_offset` + `src_cb` refers to a region outside `src_buffer`.
...
<<clEnqueueReadBuffer.adoc#, `clEnqueueReadBuffer`>>, <<clEnqueueWriteBuffer.adoc#, `clEnqueueWriteBuffer`>>, <<clCreateBuffer.adoc#, `clCreateBuffer`>>, <<clEnqueueCopyBuffer.adoc#, `clEnqueueCopyBuffer`>>, <<clEnqueueCopyImageToBuffer.adoc#, `clEnqueueCopyImageToBuffer`>>, <<cl_khr_mipmap_image.adoc#, `cl_khr_mipmap_image`>>
