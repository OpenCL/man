...
* `CL_INVALID_VALUE` if the 1D, 2D, or 3D rectangular region specified by `src_origin` and `src_origin` + `region` refers to a region outside `src_image`, or if the region specified by `dst_offset` and `dst_offset` + `dst_cb` refers to a region outside `dst_buffer`.
...
<<clEnqueueReadBuffer.adoc#, `clEnqueueReadBuffer`>>, <<clEnqueueWriteBuffer.adoc#, `clEnqueueWriteBuffer`>>, <<clCreateBuffer.adoc#, `clCreateBuffer`>>, <<clEnqueueCopyBuffer.adoc#, `clEnqueueCopyBuffer`>>, <<clEnqueueCopyBufferToImage.adoc#, `clEnqueueCopyBufferToImage`>>, <<cl_khr_mipmap_image.adoc#, `cl_khr_mipmap_image`>>
