...
If `blocking_write` is `CL_TRUE` the OpenCL implementation copies the data referred to by `ptr` and enqueues the write command in the command-queue.
The memory pointed to by `ptr` can be reused by the application after the `clEnqueueWriteImage` call returns.
...
If `input_row_pitch` is set to 0, the appropriate row pitch is calculated based on the size of each element in bytes multiplied by `width`.
...
If the mipmap extensions are enabled with <<cl_khr_mipmap_image.adoc#, `cl_khr_mipmap_image`>>, calls to <<clEnqueueReadImage.adoc#, `clEnqueueReadImage`>>, `clEnqueueWriteImage` and <<clEnqueueMapImage.adoc#, `clEnqueueMapImage`>> can be used to read from or write to a specific mip-level of a mip-mapped image.
...
`CL_DEVICE_IMAGE_SUPPORT` specified the table of allowed values for `param_name` for <<clGetDeviceInfo.adoc#, `clGetDeviceInfo`>> is `CL_FALSE`).
...
<<clEnqueueReadImage.adoc#, `clEnqueueReadImage`>>, <<clEnqueueCopyImage.adoc#, `clEnqueueCopyImage`>>, <<cl_khr_mipmap_image.adoc#, `cl_khr_mipmap_image`>>
