...
The fill color is a single floating point value if the channel order is `CL_DEPTH`.
...
The fill color will be converted to the appropriate image channel format and order associated with `image` as described in sections 6.12.14 and 8.3.
...
If the <<cl_khr_mipmap_image.adoc#, `cl_khr_mipmap_image`>> extension is enabled, calls to <<clEnqueueFillImage.adoc#, `clEnqueueFillImage`>> can be used to write to a specific mip-level of a mip-mapped image.
...
<<clEnqueueReadImage.adoc#, `clEnqueueReadImage`>>, <<clEnqueueWriteImage.adoc#, `clEnqueueWriteImage`>>, <<clEnqueueCopyImage.adoc#, `clEnqueueCopyImage`>>, <<clEnqueueBarrierWithWaitList.adoc#, `clEnqueueBarrierWithWaitList`>>
