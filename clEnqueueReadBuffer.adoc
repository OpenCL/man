...
the read command is blocking, `clEnqueueReadBuffer` does not return until the buffer data has been read and copied into memory pointed to by `ptr`.

If `blocking_read` is `CL_FALSE` i.e.
the read command is `non-blocking`, `clEnqueueReadBuffer` queues a `non-blocking` read command and returns.
...
`clEnqueueReadBuffer` returns `CL_SUCCESS` if the function is executed successfully.
...
* `CL_INVALID_VALUE` if the region being read specified by (`offset`, `size`) is out of bounds or if `ptr` is a NULL value.
...
<<clEnqueueCopyBuffer.adoc#, `clEnqueueCopyBuffer`>>, <<clEnqueueWriteBuffer.adoc#, `clEnqueueWriteBuffer`>>, <<clEnqueueWriteBufferRect.adoc#, `clEnqueueWriteBufferRect`>>, <<clEnqueueReadBufferRect.adoc#, `clEnqueueReadBufferRect`>>
