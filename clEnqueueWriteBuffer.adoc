...
* `CL_INVALID_VALUE` if the region being written specified by (`offset`, `size`) is out of bounds or if `ptr` is a NULL value.
...
* `CL_INVALID_OPERATION` if <<clEnqueueWriteBuffer.adoc#, `clEnqueueWriteBuffer`>> is called on `buffer` which has been created with `CL_MEM_HOST_READ_ONLY` or `CL_MEM_HOST_NO_ACCESS`.
...
<<clEnqueueCopyBuffer.adoc#, `clEnqueueCopyBuffer`>>, <<clEnqueueReadBuffer.adoc#, `clEnqueueReadBuffer`>>, <<clEnqueueReadBufferRect.adoc#, `clEnqueueReadBufferRect`>>, <<clEnqueueWriteBufferRect.adoc#, `clEnqueueWriteBufferRect`>>
