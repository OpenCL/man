...
Event objects are unique and can be used to identify this barrier command later on.
...
This command returns an `event` which can be waited on, i.e.
this event can be waited on to insure that all events either in the `event_wait_list` or all previously enqueued commands, queued before this command to `command_queue`, have completed.
...
<<clEnqueueFillBuffer.adoc#, `clEnqueueFillBuffer`>>, <<clEnqueueCopyBuffer.adoc#, `clEnqueueCopyBuffer`>>, <<clEnqueueCopyBufferRect.adoc#, `clEnqueueCopyBufferRect`>>, <<clEnqueueFillImage.adoc#, `clEnqueueFillImage`>>, <<clEnqueueCopyImage.adoc#, `clEnqueueCopyImage`>>, <<clEnqueueCopyImageToBuffer.adoc#, `clEnqueueCopyImageToBuffer`>>, <<clEnqueueCopyBufferToImage.adoc#, `clEnqueueCopyBufferToImage`>>, <<clEnqueueUnmapMemObject.adoc#, `clEnqueueUnmapMemObject`>>
