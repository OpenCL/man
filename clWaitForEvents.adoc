...
If the <<cl_khr_gl_event.adoc#, `cl_khr_gl_event`>> extension is enabled, event objects can also be used to reflect the status of an OpenGL sync object.
The sync object in turn refers to a fence command executing in an OpenGL command stream.
This provides another method of coordinating sharing of buffers and images between OpenGL and OpenCL.

If the <<cl_khr_egl_event.adoc#, `cl_khr_egl_event`>> extension is enabled, Event objects can also be used to reflect the status of an EGL fence sync object.
...
Returns `CL_SUCCESS` if the execution status of all events in `event_list` is `CL_COMPLETE`.
...
<<clGetEventInfo.adoc#, `clGetEventInfo`>>, <<clReleaseEvent.adoc#, `clReleaseEvent`>>, <<clRetainEvent.adoc#, `clRetainEvent`>>, <<cl_khr_gl_event.adoc#, `cl_khr_gl_event`>>, <<cl_khr_egl_event.adoc#, `cl_khr_egl_event`>>
