...
* `CL_INVALID_VALUE` if or if `lengths` or `binaries` are NULL or if any entry in `lengths`[i] or `binaries`[i] is NULL.
...
<<clCreateProgramWithSource.adoc#, `clCreateProgramWithSource`>>, <<clCreateProgramWithBuiltInKernels.adoc#, `clCreateProgramWithBuiltInKernels`>>, <<clReleaseProgram.adoc#, `clReleaseProgram`>>, <<clRetainProgram.adoc#, `clRetainProgram`>>, <<classDiagram.adoc#, Cardinality Diagram>>
