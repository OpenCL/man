...
A pointer that was specified in a previous call to <<clEnqueueSVMMap.adoc#, `clEnqueueSVMMap`>>.
...
Specify events that need to complete before `clEnqueueSVMUnmap` can be executed.
...
The above note does not apply to fine-grained SVM buffers (fine-grained buffers allocated using <<clSVMAlloc.adoc#, `clSVMAlloc`>> or fine-grained system allocations).


[[errors]]
== Errors

`clEnqueueSVMUnmap` returns `CL_SUCCESS` if the function is executed successfully.
